const plaine_normal = [new Point(1,2),new Point(1,3),new Point(1,4),new Point(1,5),new Point(1,6),new Point(1,13),new Point(1,14),new Point(2,4)]
const plaine_colline = [new Point(1,7),new Point(1,8)]
const plaine_montagne = [new Point(1,9),new Point(1,10),new Point(1,11),new Point(1,12)]

const jungle_normal = [new Point(2,1),new Point(2,2), new Point(2,3), new Point(2,5), new Point(2,9), new Point(2,10)]
const jungle_colline = [new Point(2,5), new Point(2,11)]
const jungle_montagne = [new Point(2,6), new Point(2,7), new Point(2,8)]

const savane_normal = [new Point(3,1),new Point(3,2),new Point(3,3),new Point(3,4),new Point(3,5), new Point(3,11),new Point(3,12)]
const savane_colline = [new Point(3,6),new Point(3,7), new Point(3,8)]
const savane_montagne = [new Point(3,9), new Point(3,10)]

const neige_normal = [new Point(4,1),new Point(4,6),new Point(4,7),new Point(4,8)]
const neige_colline = [new Point(4,2),new Point(4,3)]
const neige_montagne = [new Point(4,4),new Point(4,5),new Point(4,9),new Point(4,10)]

const desert_normal = [new Point(5,1),new Point(5,2),new Point(5,3), new Point(5,7)]
const desert_colline = [new Point(5,4)]
const desert_montagne = [new Point(5,5), new Point(5,6)]